...
let helpDetails = {
    ...
    settings: 'Command /settings\n\nThis command lets you customize some aspects of your channels. It has five components:\n\n'
        + Object.entries({
            ...
            'Update permissions': 'This setting retrieves admins\' permissions from Telegram so that the permissions can be used with the bot. Two permissions are retrieved: Post messages and Change channel info. The Post messages is used to decide if the person is allowed to post an item using the bot, and the Change channel info is used to decide if they are allowed to change channel settings (including this one). However, the person who is registered as the admin of the channel here will not be affected.'
