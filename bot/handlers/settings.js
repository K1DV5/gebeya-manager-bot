...
async function handleSettingIntro(ctx) {
    ...
    ctx.people.set(username, {conversation: currentOne.next})
    ...
    if (channels.length > 1) {
        ...
        ctx.people.set(username, {settings_channel: channel})
...
async function handleSettingChannel(ctx) {
    ...
    ctx.people.set(username, {conversation: currentOne.next, settings_channel: channel})
    let chatId = ctx.update.callback_query.from.id
    let messageId = ctx.update.callback_query.message.message_id
    let text = await currentOne.text(ctx, channel)
    ctx.telegram.editMessageText(chatId, messageId, undefined, text, {parse_mode: 'html'})
...
async function handleSettingTextContactText(ctx) {
    ...
    let channel = await ctx.people.get(username, 'settings_channel')
